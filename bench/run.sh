...
record_environment() {
  {
    ...
    printf 'python=%s\n' "$(python3 --version)"
    # "derived" is the honest record when nothing pinned the queue: serve sizes
    # it from worker_threads, so writing a literal 256 here would describe a run
    # that did not happen once the worker count moved.
    printf 'worker_threads=%s\nmax_pending_requests=%s\n' "${QUACKAPI_WORKER_THREADS:-32}" "${QUACKAPI_MAX_PENDING_REQUESTS:-derived}"
