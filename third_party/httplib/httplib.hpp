...
// submodule commit, with the RFC 7230 §3.3.3 empty-body fix applied at
// Server::read_content_core (treat request bodies as zero-length when neither
// Content-Length nor Transfer-Encoding is present — avoids hanging keep-alive
...
inline bool Server::read_content_core(
  ...
  // RFC 7230 §3.3.3: for a request, if neither Transfer-Encoding nor
  // Content-Length is present, the message body length is zero. Reading until
  // EOF (read_content_without_length) hangs keep-alive clients that send e.g.
  // `curl -X POST` without -d / Content-Length — the handler never runs until
  // the client times out and closes, then the response is written into a dead
  // socket while access logs still record the eventual 200.
  // DELETE already had this short-circuit; extend it to all body-capable
  // methods so empty POST/PUT/PATCH (query-param-only write routes, COPY
  // decision endpoints, etc.) return immediately instead of hanging.
  if (!req.has_header("Content-Length") &&
