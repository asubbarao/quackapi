...
ParserExtensionParseResult RouteDdlParse(ParserExtensionInfo *, const string &query) {
	...
	// Optional clauses: STATUS / REQUIRE / GROUP / RATE LIMIT / FORMAT / TIMEOUT (any order)
	int status = 200;
	string require_auth;
	string group_name;
	int rate_limit_n = 0;
	...
	for (int clause_round = 0; clause_round < 14; clause_round++) {
...
unique_ptr<FunctionData> ApplyRouteBind(ClientContext &, TableFunctionBindInput &input,
	...
	if (input.inputs.size() > 15 && !input.inputs[15].IsNull()) {
		bind_data->route.timeout_sec = input.inputs[15].GetValue<int32_t>();
	}
...
TableFunction MakeApplyRouteFunction() {
	...
	// params_json, body_schema, group_name, rate_n, rate_per, rate_by, format, timeout_sec
